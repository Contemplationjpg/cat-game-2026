...
public class Camera
{
	...
	/**rotates camera around own x-axis by (float) degrees */
	public void pitch(float rot) {
		Vector3f rightVector = getU();
		Vector3f upVector = getV();
		Vector3f fwdVector = getN();
		upVector.rotateAxis(rot, rightVector.x(), rightVector.y(), rightVector.z());
		fwdVector.rotateAxis(rot, rightVector.x(), upVector.y(), rightVector.z());
        setU(upVector);
        setN(fwdVector);
    }

	/**rotates camera around own y-axis by (float) degrees */
	public void yaw(float rot) {
		Vector3f rightVector = getU();
		Vector3f upVector = getV();
		Vector3f fwdVector = getN();
		rightVector.rotateAxis(rot, upVector.x(), upVector.y(), upVector.z());
		fwdVector.rotateAxis(rot, upVector.x(), upVector.y(), upVector.z());
        setU(rightVector);
        setN(fwdVector);
    }


	/**rotates camera around world's y-axis by (float) degrees */
