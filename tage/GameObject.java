...
public class GameObject
{
	...
	*/
	...
	/** creates new rotation transformation matrix of given rotation (float) degrees around own y-axis, then uses setLocalRotation to set local rotation to current local rotation multiplied by new matrix*/
	public void yaw (float rot) {
		this.setLocalRotation(this.getLocalRotation().mul((new Matrix4f()).rotation(rot, 0, 1, 0)));
	}
	/** creates new rotation transformation matrix of given rotation (float) degrees around world's y-axis, then uses setLocalRotation to set local rotation to current local rotation multiplied by new matrix*/
	public void globalYaw (float rot) {
		this.setLocalRotation(this.getWorldRotation().mul((new Matrix4f()).rotation(rot, 0, 1, 0)));
	}
	/** creates new rotation transformation matrix of given rotation (float) degrees around own x-axis, then uses setLocalRotation to set local rotation to current local rotation multiplied by new matrix*/
